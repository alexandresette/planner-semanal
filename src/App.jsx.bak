...
/* ─── Project Card ─── */
function ProjectCard({project,onToggleTask,onUpdateTask,onAddTask,onEditProject,isExpanded,onToggleExpand,reorderMode,onMoveUp,onMoveDown,isFirst,isLast,taskMoveWeek}){
...
export default function App(){
  ...
  const taskMoveWeekFn=(pid,tid)=>{
    if(isCurrentWeek&&weeks.length>1) return(<button onClick={()=>moveTaskToWeek(0,1,pid,tid)} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px",borderRadius:8,border:"1px solid rgba(245,158,11,0.3)",background:"rgba(245,158,11,0.1)",color:"#FCD34D",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:F,width:"100%",justifyContent:"center"}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#FCD34D" strokeWidth="2.5" strokeLinecap="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>Adiar para próxima semana</button>);
    if(!isCurrentWeek) return(<button onClick={()=>moveTaskToWeek(activeWeekIdx,0,pid,tid)} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px",borderRadius:8,border:"1px solid rgba(16,185,129,0.3)",background:"rgba(16,185,129,0.1)",color:"#6EE7B7",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:F,width:"100%",justifyContent:"center"}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#6EE7B7" strokeWidth="2.5" strokeLinecap="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 5 5 12 12 19"/></svg>Adiantar para semana atual</button>);
    return null;
  };
    ...
    <div style={{minHeight:"100vh",background:c.bg,fontFamily:F,transition:"background 0.3s ease"}}>
      ...
      <div style={{maxWidth:layoutMode==="columns"?1200:520,margin:"0 auto",padding:"24px 16px 40px",transition:"max-width 0.3s ease"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
          ...
          {viewMode==="category"&&(<button onClick={()=>setReorderMode(!reorderMode)} style={{background:reorderMode?"rgba(59,130,246,0.15)":"rgba(255,255,255,0.04)",border:`1px solid ${reorderMode?"rgba(59,130,246,0.3)":"rgba(255,255,255,0.07)"}`,borderRadius:10,padding:"9px 10px",cursor:"pointer"}}>{layoutMode==="columns"?(<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={reorderMode?"#60A5FA":"#64748B"} strokeWidth="2" strokeLinecap="round"><polyline points="3 12 21 12"/><polyline points="7 8 3 12 7 16"/><polyline points="17 8 21 12 17 16"/></svg>):(<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={reorderMode?"#60A5FA":"#64748B"} strokeWidth="2" strokeLinecap="round"><polyline points="12 3 12 21"/><polyline points="8 7 12 3 16 7"/><polyline points="8 17 12 21 16 17"/></svg>)}</button>)}
