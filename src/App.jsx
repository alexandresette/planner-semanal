    ...
    <div style={{minHeight:"100vh",background:c.bg,fontFamily:F,transition:"background 0.3s ease"}}>
      ...
      <div style={{maxWidth:layoutMode==="columns"?1200:520,margin:"0 auto",padding:"24px 16px 40px",transition:"max-width 0.3s ease"}}>
        ...
        <div style={{background:isCurrentWeek?c.weekBg:c.weekNextBg,border:`1px solid ${c.cardBorder}`,borderRadius:16,padding:"16px 20px",marginBottom:16,transition:"all 0.3s ease"}}>
          ...
          {(()=>{const highTasks=projects.flatMap(p=>p.tasks.filter(t=>t.priority==="high"&&!t.done).map(t=>({...t,_projectEmoji:p.emoji,_projectColor:p.color})));if(highTasks.length===0)return null;return(<div style={{marginTop:14,paddingTop:12,borderTop:`1px solid ${c.divider}`}}><div onClick={()=>setFocosOpen(o=>!o)} style={{display:"flex",alignItems:"center",gap:6,marginBottom:focosOpen?8:0,cursor:"pointer",userSelect:"none"}}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#EF4444" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg><span style={{fontSize:11,fontWeight:700,color:"#EF4444",fontFamily:F,textTransform:"uppercase",letterSpacing:"0.06em"}}>Focos da semana</span><span style={{fontSize:10,fontWeight:600,color:"#EF4444",background:"rgba(239,68,68,0.12)",borderRadius:5,padding:"1px 6px",fontFamily:F}}>{highTasks.length}</span><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#EF4444" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{marginLeft:"auto",transition:"transform 0.2s ease",transform:focosOpen?"rotate(180deg)":"rotate(0deg)"}}><polyline points="6 9 12 15 18 9"/></svg></div>{focosOpen&&<div style={{display:"flex",flexDirection:"column",gap:5,animation:"fadeIn 0.2s ease"}}>{highTasks.map((t,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:6,height:6,borderRadius:"50%",background:t._projectColor,flexShrink:0}}/><span style={{fontSize:12,color:c.textSub,fontFamily:F,lineHeight:1.4,flex:1}}>{t.text}</span><span style={{fontSize:10,fontFamily:F,color:t._projectColor,background:`${t._projectColor}18`,padding:"1px 6px",borderRadius:4,whiteSpace:"nowrap"}}>{t._projectEmoji}</span></div>))}</div>}</div>);})()}
        </div>
