  ...
  return(
    <div className="project-card" style={{background:"rgba(255,255,255,0.04)",borderRadius:16,border:"1px solid rgba(255,255,255,0.07)",overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"16px 16px 16px 20px"}}>
        ...
        {editingEmoji?(<input autoFocus value={emojiVal} onChange={e=>setEmojiVal(e.target.value)} onClick={e=>e.stopPropagation()} onKeyDown={e=>{if(e.key==="Enter")saveEmoji();if(e.key==="Escape"){setEmojiVal(project.emoji);setEditingEmoji(false);}}} onBlur={saveEmoji} style={{width:42,height:42,fontSize:22,textAlign:"center",borderRadius:10,background:"rgba(255,255,255,0.06)",border:`2px solid ${project.color}`,color:"#F1F5F9",outline:"none",padding:0,fontFamily:F}}/>):(<div onClick={e=>{e.stopPropagation();setEditingEmoji(true);}} style={{fontSize:28,cursor:"pointer"}}>{project.emoji}</div>)}
        <div style={{flex:1,cursor:"pointer"}} onClick={onToggleExpand}>
          <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><span style={{fontSize:16,fontWeight:700,color:"#F1F5F9",fontFamily:F}}>{project.name}</span>{project.deadline&&(<span style={{fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:6,background:"rgba(239,68,68,0.15)",color:"#FCA5A5",fontFamily:F}}>prazo: {project.deadline}</span>)}</div>
